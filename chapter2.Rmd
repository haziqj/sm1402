...
library(tidyverse)
# devtools::install_github("jweisber/ptree")
library(ptree)
```
...
nodes <- bind_rows(
  list(text = "T", parent = 0, p = 2/3),
  list(text = "T'", parent = 0, p = 1/3),
  list(text = "G", parent = 1, p = 0.8),
  list(text = "G'", parent = 1, p = 0.2),
  list(text = "G", parent = 2, p = 0.5),
  list(text = "G'", parent = 2, p = 0.5  )
...
.center[
```{r, echo = FALSE, fig.retina = 3, fig.height = 3.4, fig.width = 6, out.width = 1100, warning = FALSE}
bday.fn <- function(x) {
  tmp <- exp(lfactorial(365) - lfactorial(365 - x) - x * log(365))
  1 - tmp
}
dat <- data.frame(
  x = 2:100,
...
ggplot(dat, aes(x, y)) +
  geom_step() +
  labs(x = "Number of people", y = "Probability at least two share birthday") +
  geom_segment(x = -10, xend = 12, y = 0.5, yend = 0.5, col = "red",
               linetype = "dashed", arrow = the.arrow) +
  geom_segment(x = 12, xend = 23, y = 0.5, yend = 0.5, col = "red",
               linetype = "dashed") +
  geom_segment(x = 23, xend = 23, y = 0.5, yend = 0.2, col = "red",
               linetype = "dashed", arrow = the.arrow) +
  geom_segment(x = 23, xend = 23, y = 0.2, yend = -10, col = "red",
               linetype = "dashed") +  
  geom_text(aes(x = 9, y = 0.57, label = "50% chance"), col = "red") +
  geom_text(aes(x = 32, y = 0, label = "23 people"), col = "red") +
  geom_segment(x = -10, xend = 25, y = 0.999, yend = 0.999, col = "red",
               linetype = "dashed", arrow = the.arrow) +
  geom_segment(x = 25, xend = 70, y = 0.999, yend = 0.999, col = "red",
               linetype = "dashed") +
  geom_segment(x = 70, xend = 70, y = 0.999, yend = 0.5, col = "red",
               linetype = "dashed", arrow = the.arrow) +
  geom_segment(x = 70, xend = 70, y = 0.5, yend = -10, col = "red",
               linetype = "dashed") +
  geom_text(aes(x = 9, y = 0.95, label = "99.9% chance"), col = "red") +
  geom_text(aes(x = 79, y = 0, label = "70 people"), col = "red") +
  theme_bw()
```
]
...
```{r, include = FALSE}
file_name <- paste0("file://", normalizePath("chapter2.html"))
# webshot::webshot(file_name, "chapter2.pdf", delay = 5)
pagedown::chrome_print(file_name, wait = 3)
```
